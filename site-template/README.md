...
Edit **`.env`**: set `BASE_URL`, `SESSION_SECRET`, `DISCORD_CLIENT_ID`, `DISCORD_CLIENT_SECRET`, optional `DISCORD_BOT_TOKEN`, `HELIUS_API_KEY`, `TOKEN_MINT` / `BLUNA_TOKEN_MINT`, and any collection mints.
...
In **`server.js`**, the `COLLECTIONS` array (and token mint) can be driven by env. Set `TOKEN_MINT` / `BLUNA_TOKEN_MINT` and e.g. `MNK3YS_COLLECTION_MINT`, `ZMB3YS_COLLECTION_MINT` for your collections, or edit `COLLECTIONS` to match your slugs/names. Keep the same keys as in `holdingsLabels` / `holdersSortOptions` (e.g. `mnk3ys`, `zmb3ys`) so labels stay in sync.
